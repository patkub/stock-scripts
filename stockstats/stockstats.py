...
for symbol in args.symbols:
    ...
    try:
        ...
        current_price = (
            float(current_price["2. high"]) + float(current_price["3. low"])
        ) / 2
        ...
        display = "{0}: Current: {1:.2f} Bands: {2:.2f} ({3:+.2f}%), {4:.2f} ({5:+.2f}%), {6:.2f} ({7:+.2f}%)".format(
            symbol,
            current,
            lower,
            percent_lower_current,
            mid,
            percent_mid_current,
            upper,
            percent_upper_current,
        )
        print(display)
